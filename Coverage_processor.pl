#!/usr/bin/perl
# This script uses "coverage" file to perform sertain operations on specified subsets of genes.
# WARNING!!! In this version I disabled interval based normalization. (scroll down for $end_row_sum).
# Usage:
# Coverage_processor.pl ARGV[0] ARGV[1] ARGV[2] ARGV[3]
# Where ARGV[0] - numerical, length of the CDSs used to select the subset, recommended 2000 for mice
#       ARGV[1] - dichotomous "start" or "stop", specifies the location of the ribosome density profile
#       ARGV[2] - file name (usually "coverage")
#       ARGV[3] - path to the output folder
# extract genes with specified length (centered either at start or stop codon)
open (FILE, "<$ARGV[2]"); open (OUT, ">".$ARGV[3]."temp1"); 
while ($line = <FILE>){
     chomp($line);
     @line = split /\t/, $line;  
...
while ($line = <FILE>)
   {
     ...
     #$end_row_sum = 0; # gene normalization factor based on the average coverage across the [ARGV[0] - 400nt] interval
...
unlink "$ARGV[3]"."temp1"; unlink "$ARGV[3]"."temp2"; # these files were only created for manual check
