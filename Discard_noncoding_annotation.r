...
# # mouse
# infile = "/icgc/dkfzlsdf/analysis/OE0532/static/elongation/mm10/GRCm38.p6.Refseq.gff"
# outfile = "/icgc/dkfzlsdf/analysis/OE0532/static/elongation/mm10/GRCm38.p6.Refseq.coding.gff"
# 
# # human
# infile = "/icgc/dkfzlsdf/analysis/OE0532/static/elongation/hg38/GRCh38.p13.Refseq.gff"
# outfile = "/icgc/dkfzlsdf/analysis/OE0532/static/elongation/hg38/GRCh38.p13.Refseq.coding.gff"

gff    <- fread(file=infile, stringsAsFactors = F, header = F, fill = T, na.strings = c("", "NA"), sep="\t") %>% na.omit() #deals with unwanted #comment lines in the gff
...
# con <- file("/icgc/dkfzlsdf/analysis/OE0532/static/elongation/GRCm38.p6.Refseq.gff", "r")
# header <- readLines(con, n = 7)
# write.table(header, file = outfile, col.names = F, row.names = F, quote = F)
print(paste('Writing file', outfile))
write.table(gff2[,1:9], file = outfile, sep = "\t", row.names = F, col.names = F, quote = F, append = T)
close(con); rm(con)
