#!/usr/bin/perl
# Usage: BLASTNprocessor.pl blast_result.txt
...
foreach my $name (sort keys %hash)   {
     my $counter = 0;
     if(scalar(@{$hash{$name}}) == 1 ) {  print OUT2 $hash{$name}[0];  $check_hash{$name} = "unique";}
     else {
       for (my $i = 1; $i < scalar(@{$hash{$name}}); $i++){
       ...
       if ($counter == 0) { print OUT2 $hash{$name}[0];  $check_hash{$name} = "unique";}                
       else {  foreach $element (@{$hash{$name}}) {print OUT1 $element;}}
     }
}
