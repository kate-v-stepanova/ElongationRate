...
infile = sys.argv[1] # this one: /icgc/dkfzlsdf/analysis/OE0532/static/elongation/hg38/GRCh38.p13.rna.gbk

# extract RefSeq records
...
df = df.sort_values(on=['gene_name', 'CDS_length', '5UTR_length', '3UTR_length'])
...
#print "$ARGV[0]\n";
#
#open (INPUT, "<$ARGV[0]"); open (OUT, ">/icgc/dkfzlsdf/analysis/OE0532/static/elongation/temp1");
#print OUT "gene_name\tRefSeq:ID\tCDS_length\t5UTR_length\t3UTR_length\tSequence\n";
#while($line = <INPUT>)
#   {
#      print $line;
#      @line=split /\s+/,$line;
#      print index($line[1], "NM_");
...
#             }
#      else { print "else: $line"; until (index($line, '//') != -1) {$line = <INPUT>;} }
#   }
#close (INPUT); close (OUT);
#
####select the longest CDS's for every gene. If CDS are equal, the one with the longest 5'-UTR is chosen. If 5`-UTR are also equal, then 3'-UTR is evaluated.
#open (INPUT, "</icgc/dkfzlsdf/analysis/OE0532/static/elongation/temp1"); open (OUT, ">/icgc/dkfzlsdf/analysis/OE0532/static/elongation/temp2");
...
#open (INPUT, "</icgc/dkfzlsdf/analysis/OE0532/static/elongation/temp2"); open (OUT, ">/icgc/dkfzlsdf/analysis/OE0532/static/elongation/temp3");
